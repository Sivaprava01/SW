...
export function LogTransactionModal({ visible, onClose, onSave }: LogTransactionModalProps) {
  ...
  const baseSurplus = 4200;
  const projectedSurplus = txType === 'income' ? baseSurplus + numAmount : baseSurplus - numAmount;
  ...
  const incomeCategories = [
    ...
    { name: 'SHG Disbursed', icon: 'diversity-3' as const },
    { name: 'Govt Benefit', icon: 'assured-workload' as const },
    { name: 'Other Income', icon: 'savings' as const },
  ];

  const expenseCategories = [
    { name: 'Food & Groceries', icon: 'shopping-cart' as const },
    { name: 'Rent & Utility', icon: 'lightbulb' as const },
    { name: 'Healthcare', icon: 'medical-services' as const },
    { name: 'School Fees', icon: 'school' as const },
    { name: 'Loan Repayment', icon: 'assignment-return' as const },
