...
export default function InteractiveTutorial({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm sm:max-w-md max-h-[94vh] flex flex-col overflow-hidden shadow-2xl border border-amber-100 dark:border-slate-800 animate-in zoom-in-95">
        
        {/* Top Meta Bar: Step Pill + Skip Action (Stitch 1:1) */}
        <div className="px-4 py-3 border-b border-amber-100/60 dark:border-slate-800 flex items-center justify-between bg-[#fffaf5] dark:bg-slate-900 shrink-0">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#fff1e3] dark:bg-slate-800 text-orange-950 dark:text-orange-200 border border-orange-200/60 dark:border-slate-700 shadow-2xs">
          ...
          <button
            ...
            className="text-xs font-bold text-stone-400 hover:text-stone-600 dark:hover:text-stone-200 p-1 cursor-pointer transition"
          >
            Skip Tour
          </button>
        </div>
        ...
        {/* Progress Bar Fill */}
        <div className="w-full bg-stone-100 dark:bg-slate-800 h-1.5 shrink-0">
          <div
        ...
        {/* Main Step Card Body */}
        <div className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-[#fffaf5]/40 dark:bg-slate-950 text-xs sm:text-sm">
          
          {/* Hero Visual Container */}
          <div className="w-full rounded-2xl bg-gradient-to-b from-[#fff1e3] to-white dark:from-slate-800 dark:to-slate-900 p-3.5 flex flex-col items-center justify-center relative overflow-hidden border border-orange-200/50 dark:border-slate-800 shadow-2xs">
            <div className="w-14 h-14 rounded-2xl bg-orange-100 dark:bg-orange-950/80 flex items-center justify-center text-orange-700 dark:text-orange-300 shadow-xs mb-2">
              <Icon size={28} />
            </div>

            {/* Authentic Photograph */}
            <div className="w-full h-32 rounded-xl overflow-hidden relative shadow-xs border border-amber-100/80 dark:border-slate-700">
              <img
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end p-2.5">
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-white/90 dark:bg-slate-900/90 text-orange-700 dark:text-orange-300 font-black text-[10px] shadow-sm">
          ...
          <div className="space-y-0.5">
            <h2 id="tutorial-title" className="text-base sm:text-lg font-black text-stone-900 dark:text-stone-100 tracking-tight leading-snug whitespace-pre-line">
              {step.title}
            </h2>
          ...
          <p className="text-xs text-stone-600 dark:text-stone-300 leading-relaxed bg-white dark:bg-slate-900 p-3 rounded-xl border border-amber-100 dark:border-slate-800">
            {step.body}
          </p>

          {/* Audio Preview Pill with Live TTS Voice */}
          <div className="w-full rounded-xl bg-white dark:bg-slate-900 p-2.5 border border-amber-100 dark:border-slate-800 flex items-center justify-between gap-2 shadow-2xs">
              ...
              {isTtsSupported && (
                <button
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center shadow-xs cursor-pointer active:scale-90 transition shrink-0 ${
                    isStepSpeaking
                      ? 'bg-amber-100 dark:bg-amber-950 text-amber-900 dark:text-amber-300 border border-amber-300 animate-bounce'
            ...
            <span className="px-2 py-0.5 rounded-full bg-orange-50 dark:bg-slate-800 text-orange-800 dark:text-orange-300 text-[10px] font-bold border border-orange-200/50 dark:border-slate-700 shrink-0">
              {step.duration}
            </span>
          </div>

          {/* Reassurance Tip Box */}
          <div className="w-full rounded-xl bg-[#fff1e3]/60 dark:bg-slate-800/60 p-3 border border-orange-200/60 dark:border-slate-700 flex items-start gap-2 text-stone-800 dark:text-stone-200">
          ...
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-[11px] font-bold text-stone-500 dark:text-stone-400 px-1">
              <span>Your 5-Step Path</span>
              <span className="text-orange-600 dark:text-orange-400">{step.step} of 5</span>
            </div>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-0.5 p-1.5 rounded-xl transition cursor-pointer min-h-[44px] justify-center border ${
                      isActive
                        ...
                        : isPast
                        ? 'bg-[#fff1e3] dark:bg-slate-800 text-orange-950 dark:text-orange-200 border-orange-200/50 dark:border-slate-700'
                        : 'bg-white dark:bg-slate-900 text-stone-400 border-stone-200/60 dark:border-slate-800'
                    }`}
                  >
        ...
        {/* Bottom Navigation Controls (Stitch 1:1) */}
        <div className="p-3.5 border-t border-amber-100/60 dark:border-slate-800 bg-[#fffaf5] dark:bg-slate-900 flex items-center justify-between gap-2.5 shrink-0">
          <button
            type="button"
            onClick={handleBack}
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-1 min-h-[44px] cursor-pointer ${
              currentStep === 0
                ? 'opacity-40 bg-stone-100 dark:bg-slate-800 text-stone-400 cursor-not-allowed'
                : 'bg-stone-200 hover:bg-stone-300 dark:bg-slate-800 dark:hover:bg-slate-750 text-stone-800 dark:text-stone-200 active:scale-95'
            }`}
          >
