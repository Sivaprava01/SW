...
export default function Onboarding({ onComplete }) {
  ...
  const handleAudioGuide = () => {
    if (isSpeaking) {
      ...
      speak(t('onboarding_audio_guide'), 'onboarding-guide', language);
    }
  };

  const handleToggleLanguage = () => {
    const nextLang = language === 'te' ? 'hi' : language === 'hi' ? 'en' : 'te';
      ...
      <div className="px-3.5 sm:px-4 pt-3 flex flex-col gap-4 max-w-md mx-auto w-full">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className={`flex flex-col gap-1.5 items-center text-center ${step === 1 ? '' : 'opacity-65'}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-bold text-orange-700 dark:text-[#ffb690] truncate">{t('onboarding_step_personal')}</span>
              </div>
            </div>
            ...
            <div className={`flex flex-col gap-1.5 items-center text-center ${step === 2 ? '' : 'opacity-65'}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-bold text-stone-700 dark:text-[#D4C4B5] truncate">{t('onboarding_step_cashflow')}</span>
              </div>
            </div>
            ...
            <div className={`flex flex-col gap-1.5 items-center text-center ${step === 3 ? '' : 'opacity-65'}`}>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs font-bold text-stone-700 dark:text-[#D4C4B5] truncate">{t('onboarding_step_dream')}</span>
              </div>
            </div>
          </div>
        </nav>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
            ...
            <div className="space-y-3.5 animate-in fade-in">
              ...
              <div className="bg-white dark:bg-[#1e1b19] rounded-2xl p-4 shadow-xs flex flex-col gap-2 border border-amber-200/70 dark:border-[#3D332B]">
                <label className="text-xs font-bold text-stone-800 dark:text-[#FFF5EB] flex items-center justify-between" htmlFor="fullNameInput">
                  ...
                  <span className="text-orange-600 dark:text-[#ffb690] text-[10px] uppercase font-bold">{t('onboarding_step_1_of_3')}</span>
                </label>
              ...
              <div className="bg-white dark:bg-[#1e1b19] rounded-2xl p-4 shadow-xs flex flex-col gap-2.5 border border-amber-200/70 dark:border-[#3D332B]">
                <label className="text-xs font-bold text-stone-800 dark:text-[#FFF5EB]">
                  {t('onboarding_location_question')}
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
            ...
            <div className="space-y-3.5 animate-in fade-in">
              <div className="bg-white dark:bg-[#1e1b19] rounded-2xl p-4 shadow-xs flex flex-col gap-3.5 border border-amber-200/70 dark:border-[#3D332B]">
                <h2 className="text-sm font-bold text-stone-900 dark:text-[#FFF5EB] flex items-center justify-between">
                  <span>{t('onboarding_step2_title')}</span>
                  <span className="text-orange-600 dark:text-[#ffb690] text-[10px] uppercase font-bold">{t('onboarding_step_2_of_3')}</span>
                </h2>
          ...
          {step === 3 && (
            <div className="space-y-3.5 animate-in fade-in">
              <div className="bg-white dark:bg-[#1e1b19] rounded-2xl p-4 shadow-xs flex flex-col gap-3.5 border border-amber-200/70 dark:border-[#3D332B]">
                <h2 className="text-sm font-bold text-stone-900 dark:text-[#FFF5EB] flex items-center justify-between">
                  <span>{t('onboarding_step3_title')}</span>
                  <span className="text-orange-600 dark:text-[#ffb690] text-[10px] uppercase font-bold">{t('onboarding_step_3_of_3')}</span>
                </h2>

                <div>
                  <label className="block text-xs font-bold text-stone-700 dark:text-[#D4C4B5] mb-1">
                    {t('onboarding_existing_savings')}
                  </label>
                  <div className="relative flex items-center">
                    <span className="absolute left-3.5 text-stone-400 font-bold">₹</span>
                    <input
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              <span>
                {step === 3
                  ? (loading ? t('onboarding_creating') : t('onboarding_start_journey'))
                  : (step === 1 ? t('onboarding_next_cashflow') : t('onboarding_next_dream'))}
              </span>
        ...
        <div className="flex items-center justify-center gap-2 pt-1 pb-4">
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-bold active:scale-95 transition cursor-pointer border shadow-2xs ${
              isSpeaking
                ? 'bg-orange-600 text-white border-orange-600 animate-pulse'
                : 'bg-[#fcebd7] dark:bg-[#28211C] text-stone-700 dark:text-[#D4C4B5] border-amber-200/60 dark:border-[#3D332B] hover:bg-orange-100'
            }`}
          >
            <IconVolume size={15} className={isSpeaking ? 'text-white' : 'text-orange-600'} />
            <span>
              {isSpeaking
                ? t('stop_listening')
                : (language === 'te' ? 'వినండి (తెలుగు)' : language === 'hi' ? 'सुनें (हिंदी)' : 'Listen (Guide)')}
            </span>
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-[#fcebd7] dark:bg-[#28211C] text-stone-700 dark:text-[#D4C4B5] text-xs font-bold active:scale-95 transition cursor-pointer border border-amber-200/60 dark:border-[#3D332B] hover:bg-orange-100 shadow-2xs"
          >
            <IconLanguage size={15} className="text-rose-700 dark:text-rose-400" />
            <span>
              {language === 'te' ? 'భాష: తెలుగు' : language === 'hi' ? 'भाषा: हिंदी' : 'Language: English'}
            </span>
          </button>
        </div>
      </div>
