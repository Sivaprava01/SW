...
type EligibilityMatcherModalProps = {
  visible: boolean;
  onClose: () => void;
  onComplete?: (results: { occupation: string; state: string }) => void;
};

export function EligibilityMatcherModal({
  visible,
  onClose,
  onComplete,
}: EligibilityMatcherModalProps) {
  ...
  const audioTrackId = `matcher-step-${step}`;
  ...
  const handleToggleAudio = async () => {
    ...
    try {
      ...
      if (__DEV__) console.warn('[EligibilityMatcherModal] Failed to play audio:', err);
      setIsLoadingAudio(false);
    }
  };
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      try {
        ...
        // Persist preferences to live backend
        ...
        if (onComplete) {
          onComplete({ occupation: selectedOccupation, state: selectedState });
        }
      } catch (err) {
        if (__DEV__) console.warn('[EligibilityMatcherModal] Error saving preferences:', err);
      } finally {
