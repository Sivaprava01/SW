...
type CreateGoalModalProps = {
  ...
  onSaveGoal?: (goal: GoalResponse) => void;
};
...
export function CreateDreamPotModal({ visible, onClose, initialGoal, onSaveGoal }: CreateGoalModalProps) {
  ...
  useEffect(() => {
    if (initialGoal) {
      setSelectedCategory(initialGoal.category || 'Education');
      setGoalTitle(initialGoal.name);
      setTargetAmount(initialGoal.target_amount);
      setDurationMonths(initialGoal.target_months || 12);
    } else {
      setSelectedCategory('Education');
      setGoalTitle("Daughter's College Admission");
  ...
  const categories = [
    ...
    { id: 'Emergency Fund', label: 'Emergency Shield', icon: 'shield' as const },
    { id: 'Small Business', label: 'Business & Sewing', icon: 'storefront' as const },
    { id: 'Gold & Jewelry', label: 'Gold / Jewelry', icon: 'savings' as const },
    { id: 'Home Improvement', label: 'House Repair', icon: 'home-repair-service' as const },
    { id: 'General Savings', label: 'Custom Dream', icon: 'auto-awesome' as const },
  ];
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      let savedGoal: GoalResponse;
      if (initialGoal) {
        savedGoal = await updateGoal(initialGoal.id, {
          name: goalTitle.trim(),
          target_amount: targetAmount,
          target_months: durationMonths,
      ...
      Alert.alert('Error', err.message || 'Failed to save dream pot');
    } finally {
      setIsSaving(false);
    }
  };
            ...
            {isSaving ? (
              ...
              <>
                <Text className="text-sm font-bold text-on-primary mr-1">
                  {initialGoal ? 'Update Goal Details' : `Lock Goal & Save ₹${monthlyRequired.toLocaleString('en-IN')}/mo`}
                </Text>
